...
import { applyDiscovery, createDiscovery, IssuerMismatchError, needsDiscovery } from './discovery'
...
describe('issuer validation', () => {
  const at = (issuer: string) => applyDiscovery({ issuerPath: 'https://auth.com' } as any, { ...wellKnown, issuer })

  it('accepts the document that claims the issuer it was fetched from', () => {
    expect(at('https://auth.com').issuer).toBe('https://auth.com')
    expect(at('https://auth.com/').issuer).toBe('https://auth.com/')
  })

  // OIDC Discovery 1.0 §4.3. Without this a host that will serve someone else's document — a
  // multi-tenant provider, a misrouted proxy — hands over that issuer's endpoints.
  it('refuses a document claiming to be issued by someone else', () => {
    expect(() => at('https://evil.com')).toThrow(IssuerMismatchError)
    expect(() => at('https://auth.com.evil.com')).toThrow(IssuerMismatchError)
  })

  it('names both sides, because either one may be the mistake', () => {
    try {
      at('https://evil.com')
      expect.unreachable()
    } catch (error) {
      expect((error as IssuerMismatchError).expected).toBe('https://auth.com')
      expect((error as IssuerMismatchError).actual).toBe('https://evil.com')
    }
  })

  // Entra advertises a template because the tenant is unknown until a token arrives; it can never
  // match literally, and the id-token verifier resolves it per token instead.
  it('lets the multi-tenant template through', () => {
    const applied = applyDiscovery({ issuerPath: 'https://login.microsoftonline.com/common/v2.0' } as any, {
      ...wellKnown,
  ...
  it('has nothing to check when the document asserts no issuer', () => {
    expect(() => applyDiscovery({ issuerPath: 'https://auth.com' } as any, wellKnown)).not.toThrow()
  })
})

describe('authorization_response_iss_parameter_supported', () => {
  it('becomes issuerParameter, which is what makes a missing iss an error', () => {
    expect(applyDiscovery({} as any, { ...wellKnown, authorization_response_iss_parameter_supported: true }).issuerParameter).toBe(true)
    expect(applyDiscovery({} as any, { ...wellKnown, authorization_response_iss_parameter_supported: false }).issuerParameter).toBe(false)
    expect(applyDiscovery({} as any, wellKnown).issuerParameter).toBeUndefined()
  })
})
